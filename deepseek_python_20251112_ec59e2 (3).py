...
user_file_pages = {}
user_file_index = {}  # Store file index for callback data
...
async def upload_file_handler(client, message: Message):
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("📤 Send to Telegram", callback_data=f"dl_{encode_file_name(original_file_name)}")]
        ])
...
async def show_files_page(client, message: Message, user_id: int, page: int):
    ...
    # Store file index for this page
    user_file_index[user_id] = {}
    ...
    for i in range(0, len(page_files), 2):
        ...
        for j in range(2):
            if i + j < len(page_files):
                ...
                # Use encoded file name to keep callback data small
                encoded_name = encode_file_name(file_name)
                callback_data = f"dl_{encoded_name}"
                
                # Store the mapping
                user_file_index[user_id][encoded_name] = file_name
...
async def handle_callback_query(client, callback_query: CallbackQuery):
    ...
    try:
        if data.startswith("dl_"):
            ...
            encoded_name = data.replace("dl_", "")
            file_name = decode_file_name(encoded_name)
            
            if not file_name and user_id in user_file_index:
                # Try to get from stored index
                file_name = user_file_index[user_id].get(encoded_name)
            ...
            if file_name:
                ...
                try:
                    ...
                    # Generate bot download link
                    new_encoded_name = encode_file_name(file_name)
                    bot_username = (await client.get_me()).username
                    bot_download_link = f"https://t.me/{bot_username}?start=download_{new_encoded_name}"
                    
                    # Create inline keyboard with both download options
                    keyboard = InlineKeyboardMarkup([
                        [InlineKeyboardButton("🔗 Direct Download Link", url=presigned_url)],
                        [InlineKeyboardButton("🤖 Download via Bot", url=bot_download_link)],
                        [InlineKeyboardButton("📤 Send to Telegram", callback_data=f"btdl_{new_encoded_name}")]
                    ])
            ...
            encoded_name = data.replace("btdl_", "")
            file_name = decode_file_name(encoded_name)
            ...
            if user_id in user_file_index:
                del user_file_index[user_id]
            message = callback_query.message
            message.text = "/list"
            message.from_user = callback_query.from_user
