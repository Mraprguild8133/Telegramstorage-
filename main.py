    ...
    if len(message.command) > 1:
        ...
        if param.startswith("download_"):
            ...
            if file_name:
                ...
                # Create a fake message object for download handler
                class FakeMessage:
                    def __init__(self, original_message, file_name):
                        self.text = f"/download {file_name}"
                        self.from_user = original_message.from_user
                        self.reply_text = original_message.reply_text
                        self.reply_document = original_message.reply_document
                
                fake_message = FakeMessage(message, file_name)
                await download_file_handler(client, fake_message)
                return
            else:
