    ...
    if len(message.command) > 1:
        ...
        if param.startswith("download_"):
            ...
            if file_name:
                ...
                await message.reply_text(f"🚀 Starting download: {escape_html(file_name)}")
                message.text = f"/download {file_name}"
                await download_file_handler(client, message)
                return
        else:
            # Handle other start parameters
...
async def upload_file_handler(client, message: Message):
    ...
    try:
        ...
        encoded_name = encode_file_name(os.path.basename(file_path))
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("📤 Send to Telegram", callback_data=f"download_{os.path.basename(file_path)}")]
        ])
