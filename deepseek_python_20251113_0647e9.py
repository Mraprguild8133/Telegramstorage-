...
class Config:
    ...
    WASABI_ENDPOINT_URL = os.getenv("WASABI_ENDPOINT_URL", f'https://s3.{WASABI_REGION}.wasabisys.com')
...
boto_config = BotoConfig(
    ...
    tcp_keepalive=True,
    s3={'addressing_style': 'virtual'}  # Important for Wasabi compatibility
)
...
# --- Wasabi Connection Test ---
def test_wasabi_connection():
    """Test Wasabi connection and credentials"""
    try:
        print("🧪 Testing Wasabi configuration...")
        print(f"🔑 Access Key: {config.WASABI_ACCESS_KEY[:10]}...")
        print(f"🌍 Region: {config.WASABI_REGION}")
        print(f"📦 Bucket: {config.WASABI_BUCKET}")
        print(f"🔗 Endpoint: {config.WASABI_ENDPOINT_URL}")
        ...
        # Test list buckets (this verifies credentials)
        response = test_client.list_buckets()
        print(f"✅ Credentials valid! Found {len(response['Buckets'])} buckets")
        
        # Test specific bucket access
        test_client.head_bucket(Bucket=config.WASABI_BUCKET)
        print(f"✅ Bucket '{config.WASABI_BUCKET}' is accessible")
        ...
        print(f"❌ Wasabi error: {error_code} - {error_message}")
        
        if error_code == 'InvalidAccessKeyId':
            print("❌ Check your WASABI_ACCESS_KEY in .env file")
        elif error_code == 'SignatureDoesNotMatch':
            print("❌ Check your WASABI_SECRET_KEY in .env file")
            print("💡 Tip: Make sure there are no extra spaces or special characters")
        elif error_code == 'NoSuchBucket':
            print(f"❌ Bucket '{config.WASABI_BUCKET}' doesn't exist or you don't have access")
        elif error_code == 'AccessDenied':
            print(f"❌ Access denied to bucket '{config.WASABI_BUCKET}'")
        else:
            print("❌ Check your Wasabi credentials and bucket configuration")
...
# --- Initialize Boto3 Client for Wasabi with Enhanced Error Handling ---
try:
    ...
    s3_client = boto3.client(
        ...
        aws_access_key_id=config.WASABI_ACCESS_KEY,
        aws_secret_access_key=config.WASABI_SECRET_KEY,
        region_name=config.WASABI_REGION,  # Explicitly set region
        config=boto_config  # Apply extreme config
    )
    
    # Test connection immediately
    if not test_wasabi_connection():
        print("❌ Wasabi connection test failed. Please check your credentials.")
        exit(1)
    
    print("✅ Wasabi S3 client initialized successfully!")
    
except ClientError as e:
    error_code = e.response['Error']['Code']
    error_message = e.response['Error']['Message']
    print(f"❌ Failed to initialize Wasabi S3 client: {error_code} - {error_message}")
    print("💡 Possible solutions:")
    print("1. Check your WASABI_ACCESS_KEY and WASABI_SECRET_KEY in .env file")
    print("2. Verify your bucket exists in the Wasabi control panel")
    print("3. Ensure your region matches your bucket's region")
    print("4. Try using the direct endpoint: WASABI_ENDPOINT_URL=https://s3.wasabisys.com")
    exit(1)
except Exception as e:
...
user_file_pages = {}
user_file_indices = {}  # user_id -> {file_id (short UUID) -> file_name}

# --- Temporary storage for upload handler buttons (prevents long callback data) ---
# Maps short UUID -> file_name for buttons generated in upload_file_handler
upload_callback_map = {}
    ...
    try:
        if not hasattr(ultra_pyrogram_progress_callback, 'last_edit_time') or time.time() - ultra_pyrogram_progress_callback.last_edit_time > config.PROGRESS_UPDATE_INTERVAL:
            ...
            text = (
                ...
                f"**📁 {display_task}**\n"
                f"{bar} **{percentage:.1f}%**\n"
                f"**⏱️ Elapsed:** {time.strftime('%M:%S', time.gmtime(elapsed_time))}"
            )
...
async def start_command(client, message: Message):
    ...
    if len(message.command) > 1:
        ...
        if param.startswith("download_"):
            ...
            if file_name:
                await message.reply_text(f"🚀 Starting download: `{file_name}`", parse_mode=ParseMode.MARKDOWN)
                ...
                status_message = await message.reply_text(f"🔍 Searching for `{safe_file_name}`...", parse_mode=ParseMode.MARKDOWN)
                ...
                try:
                    ...
                    await message.reply_document(
                        ...
                        caption=f"✅ **ULTRA TURBO DOWNLOAD COMPLETE!**\n"
                    ...
                    if error_code == '404':
                        ...
                        error_msg = str(e)
                        await status_message.edit_text(f"❌ **S3 Error:** {error_code} - {error_msg}", parse_mode=ParseMode.MARKDOWN)
                except Exception as e:
                    error_msg = str(e)
                    await status_message.edit_text(f"❌ **An unexpected error occurred:** {error_msg}", parse_mode=ParseMode.MARKDOWN)
                finally:
    ...
    await message.reply_photo(
        ...
        caption="🚀 **ULTRA TURBO CLOUD STORAGE BOT**\n\n"
                "Experience extreme speed with our optimized parallel processing technology!\n\n"
                "➡️ **To upload:** Just send me any file (up to 10GB!)\n"
                "⬅️ **To download:** Use `/download <file_name>` or `/list` with buttons\n"
                "📋 **To list files:** Use `/list` with download buttons\n\n"
                ...
                "**📱 Online Streaming Premium:** @aprfiletolinkpremiumbot",
        parse_mode=ParseMode.MARKDOWN
    )
...
async def turbo_mode_command(client, message: Message):
    ...
    await message.reply_text(
        f"⚡ **ULTRA TURBO MODE ACTIVE**\n\n"
        f"**Max Concurrency:** {transfer_config.max_concurrency} threads\n"
        f"**Chunk Size:** {humanbytes(transfer_config.multipart_chunksize)}\n"
        f"**Multipart Threshold:** {humanbytes(transfer_config.multipart_threshold)}\n"
        f"**Max File Size:** {humanbytes(config.MAX_FILE_SIZE)}\n"
        f"**Connection Pool:** {boto_config.max_pool_connections} connections",
        parse_mode=ParseMode.MARKDOWN
    )
...
async def upload_file_handler(client, message: Message):
    ...
    try:
        ...
        # Generate bot download link (using the Base64 encoded name for the START parameter)
        original_file_name = os.path.basename(file_path)
        encoded_name_for_start = encode_file_name(original_file_name)
        bot_username = (await client.get_me()).username
        bot_download_link = f"https://t.me/{bot_username}?start=download_{encoded_name_for_start}"
        
        # *** FIX for BUTTON_DATA_INVALID ***
        # Use short UUID for callback data instead of long encoded name
        short_file_id = str(uuid.uuid4())[:8]
        upload_callback_map[short_file_id] = original_file_name
        
        safe_file_name = original_file_name
        
        # Create inline keyboard with download options (REMOVED "Download via Bot")
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("🔗 Direct Download Link", url=presigned_url)],
            [InlineKeyboardButton("📤 Send to Telegram", callback_data=f"dl_{short_file_id}")] # Use short ID here
        ])
        
        await status_message.edit_text(
            f"✅ **ULTRA TURBO UPLOAD COMPLETE!**\n\n"
            f"**📁 File:** `{safe_file_name}`\n"
            f"**📦 Size:** {humanbytes(media.file_size)}\n"
            f"**⚡ Performance:** Ultra Turbo Mode\n\n"
            f"**Download Options:**\n"
...
async def download_file_handler(client, message: Message):
    ...
    if len(message.command) < 2:
        await message.reply_text("Usage: `/download <file_name_in_wasabi>`", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    try:
        ...
        if error_code == '404':
            ...
            await status_message.edit_text("❌ **Error:** Access denied. Check your Wasabi credentials.", parse_mode=ParseMode.MARKDOWN)
        elif error_code == 'NoSuchBucket':
            await status_message.edit_text("❌ **Error:** Bucket does not exist.", parse_mode=ParseMode.MARKDOWN)
        else:
            error_msg = str(e)
            await status_message.edit_text(f"❌ **S3 Error:** {error_code} - {error_msg}", parse_mode=ParseMode.MARKDOWN)
    except Exception as e:
...
async def show_files_page(client, message: Message, user_id: int, page: int):
    ...
    # Create message text with Markdown
    text = f"📁 **Your Files (Page {page + 1}/{total_pages})**\n\n"
    
    # Initialize file index for this user if not exists
    if user_id not in user_file_indices:
        user_file_indices[user_id] = {}
    
    file_index_map = user_file_indices[user_id]
    
    # Clear old temporary mapping for this user's list
    # The map only stores the keys for the current list view, so we reset it here.
    ...
    for i in range(0, len(page_files), 2):
        ...
        for j in range(2):
            if i + j < len(page_files):
                ...
                # Use short UUID for callback data to avoid size issues
                file_id = str(uuid.uuid4())[:8]
                file_index_map[file_id] = file_name # Map short ID to full name for callback lookup
    ...
    if total_pages > 1:
        if page > 0:
            navigation_buttons.append(InlineKeyboardButton("⬅️ Previous", callback_data=f"pg_{page-1}"))
        if page < total_pages - 1:
            navigation_buttons.append(InlineKeyboardButton("Next ➡️", callback_data=f"pg_{page+1}"))
    
    if navigation_buttons:
        keyboard_buttons.append(navigation_buttons)
    
    # Add refresh button
    keyboard_buttons.append([InlineKeyboardButton("🔄 Refresh", callback_data="rf")])
...
async def handle_callback_query(client, callback_query: CallbackQuery):
    ...
    try:
        if data.startswith("dl_"):
            # Download file callback (used by both /list and /upload buttons)
            file_id = data.replace("dl_", "")
            file_name = None
            
            # 1. Check the temporary list cache (for /list command buttons)
            if user_id in user_file_indices and file_id in user_file_indices[user_id]:
                file_name = user_file_indices[user_id][file_id]
            # 2. Check the upload map (for /upload command buttons)
            elif file_id in upload_callback_map:
                file_name = upload_callback_map[file_id]
            ...
            if file_name:
                ...
                try:
                    ...
                    keyboard = InlineKeyboardMarkup([
                        [InlineKeyboardButton("🔗 Direct Download Link", url=presigned_url)],
                        [InlineKeyboardButton("📤 Send to Telegram", callback_data=f"btdl_{file_id}")] # Use original short ID
                    ])
                    
                    await callback_query.message.reply_text(
                        f"📥 **Download Options for:** `{file_name}`\n\n"
                        f"**🔗 Direct Link:** Instant download (expires in 24h)\n"
                        f"**📤 Telegram:** Get file in this chat",
                        parse_mode=ParseMode.MARKDOWN,
                        reply_markup=keyboard
                    )
                ...
                await callback_query.answer("❌ File not found. The download link may have expired or the file was deleted.", show_alert=True)
        
        elif data.startswith("btdl_"):
            # Bot download callback
            file_id = data.replace("btdl_", "")
            file_name = None
            ...
            if file_name:
                ...
                # The download_file_handler expects the file name in message.command[1:]
                # We emulate the command structure here
                message.command = ["download", file_name] 
                message.from_user = callback_query.from_user
                
                # Use a dummy Message object as the context for the download handler
                # This ensures download_file_handler has the necessary attributes
                dummy_message = Message(
                    id=message.id,
                    chat=message.chat,
                    from_user=message.from_user,
                    date=message.date,
                    text=f"/download {file_name}",
                    command=message.command 
                )
                
                # Await the handler call to ensure execution
                await download_file_handler(client, dummy_message)
            else:
            ...
            # The message object needs to be the message that contains the keyboard
            ...
            # Since the button is on a message, we create a dummy message to trigger list_files
            dummy_message = Message(
            ...
            await list_files(client, dummy_message)
            
            # Attempt to delete the old list message if it's not the one we just replied to.
            try:
                # Assuming list_files sends a new message, we delete the one the button was pressed on.
                await callback_query.message.delete()
            except Exception:
                pass # Ignore if deletion fails (e.g., if it was already edited/deleted)
