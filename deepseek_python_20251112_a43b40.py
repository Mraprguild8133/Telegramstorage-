...
async def upload_file_handler(client, message: Message):
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("📥 Download via Bot", callback_data=f"download_{os.path.basename(file_path)}")]
        ])
        ...
        if data.startswith("download_"):
            ...
            try:
                ...
                keyboard = InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton("🤖 Download via Bot", callback_data=f"bot_download_{file_name}")]
                ])
                
                await callback_query.message.reply_text(
                    f"📥 <b>Download Options for:</b> <code>{safe_file_name}</code>\n\n"
                    f"<b>🔗 Direct Link:</b> Instant download (expires in 24h)\n"
                    f"<b>🤖 Bot Download:</b> Get file through Telegram",
                    parse_mode=ParseMode.HTML,
                    reply_markup=keyboard
                )
